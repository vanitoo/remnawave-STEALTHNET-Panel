#!/usr/bin/env python3
"""
Скрипт миграции для добавления поля hwid_device_limit в таблицу tariff.
Добавляет колонку hwid_device_limit для хранения лимита устройств по HWID.
...
try:
    ...
    if 'hwid_device_limit' not in columns:
        print(f"➕ Добавляем колонку hwid_device_limit...")
        cursor.execute("ALTER TABLE tariff ADD COLUMN hwid_device_limit INTEGER DEFAULT 0")
        print(f"✓ Колонка hwid_device_limit добавлена")
        changes_made = True
        
        # Обновляем существующие записи, устанавливая лимит в 0 (безлимит)
        cursor.execute("UPDATE tariff SET hwid_device_limit = 0 WHERE hwid_device_limit IS NULL")
        print(f"✓ Существующие записи обновлены (лимит установлен в 0 - безлимит)")
    else:
        print(f"✓ Колонка hwid_device_limit уже существует")
